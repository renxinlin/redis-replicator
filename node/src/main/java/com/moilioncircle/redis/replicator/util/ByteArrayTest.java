...
public class ByteArrayTest {
    ...
    public void test() {
        ...
        for (byte b : b1) {
            bytes2.set(i, b);
            Assert.assertEquals(b, bytes2.get(i));
            i++;
        }
        Assert.assertEquals(getString(bytes2), "sdajk");

        ByteArray bytes3 = new ByteArray(bytes2.length() - 1, 10);
        ByteArray.arraycopy(bytes2, 1, bytes3, 0, bytes2.length() - 1);
        Assert.assertEquals(str.substring(1), getString(bytes3));
    }
