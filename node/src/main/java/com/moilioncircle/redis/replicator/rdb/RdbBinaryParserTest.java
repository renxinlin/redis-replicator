...
public class RdbBinaryParserTest {
    ...
    public void testParse() {
        ...
        Assert.assertEquals(253, ((ByteArrayMap) map.get("zipmap_with_big_values").getValue()).get("253bytes".getBytes()).length);
        Assert.assertEquals(254, ((ByteArrayMap) map.get("zipmap_with_big_values").getValue()).get("254bytes".getBytes()).length);
        Assert.assertEquals(255, ((ByteArrayMap) map.get("zipmap_with_big_values").getValue()).get("255bytes".getBytes()).length);
        Assert.assertEquals(300, ((ByteArrayMap) map.get("zipmap_with_big_values").getValue()).get("300bytes".getBytes()).length);
        Assert.assertEquals(20000, ((ByteArrayMap) map.get("zipmap_with_big_values").getValue()).get("20kbytes".getBytes()).length);
        ...
        for (ZSetEntry entry : zset) {
            if (entry.getElement().equals("8b6ba6718a786daefa69438148361901")) {
                Assert.assertEquals(1d, entry.getScore(), 0.0001);
            }
            if (entry.getElement().equals("cb7a24bb7528f934b841b34c3a73e0c7")) {
                Assert.assertEquals(2.37d, entry.getScore(), 0.0001);
            }
            if (entry.getElement().equals("523af537946b79c4f8369ed39ba78605")) {
                Assert.assertEquals(3.423d, entry.getScore(), 0.0001);
            }
        }
        ...
        Assert.assertEquals(true, map.containsKey("z1"));
        Assert.assertEquals(true, map.containsKey("z2"));
        Assert.assertEquals(true, map.containsKey("z3"));
        Assert.assertEquals(true, map.containsKey("z4"));
        
        Assert.assertEquals(0, map.get("key_in_zeroth_database").getDb().getDbNumber());
        Assert.assertEquals(2, map.get("key_in_second_database").getDb().getDbNumber());
        ...
        Assert.assertEquals(new Date(1671963072573L), new Date(map.get("expires_ms_precision").getExpiredMs()));
    }
    ...
    public void assertByteArray(byte[] bytes, KeyValuePair<byte[], ?> kv) {
        if (kv instanceof KeyStringValueString) {
            ...
            Assert.assertEquals(true, Arrays.equals(bytes, ksvs.getValue()));
        } else {
            Assert.fail();
        }
    }
    ...
    public void assertByteArray(byte[] bytes, KeyValuePair<byte[], ?> kv, String field) {
        if (kv instanceof KeyStringValueHash) {
            ...
            Assert.assertEquals(true, Arrays.equals(bytes, m.get(field.getBytes())));
        } else {
            Assert.fail();
        }
    }
    ...
    public void assertByteArray(byte[] bytes, KeyValuePair<byte[], ?> kv, int index) {
        if (kv instanceof KeyStringValueList) {
            ...
            Assert.assertEquals(true, Arrays.equals(bytes, ksvh.getValue().get(index)));
        } else {
            Assert.fail();
        }
    }
    ...
    public void assertContains(List<String> list, KeyValuePair<byte[], ?> kv) {
        ...
        if (kv instanceof KeyStringValueSet) {
            ...
            for (String s : source) {
                Assert.assertEquals(true, target.contains(s));
            }
        } else {
            Assert.fail();
        }
